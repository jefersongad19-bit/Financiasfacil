...
const DB = {
  ...
  getPar(){
    const a=this.get('par'); let fix=false;
    for(const p of a){
      const n=p.nParcelas||1;
      if(!Array.isArray(p.pagamentos)){ p.pagamentos=Array(n).fill(null).map(()=>({status:'pendente',dataPagamento:null,valorPago:null})); fix=true; }
      else if(p.pagamentos.length<n){ while(p.pagamentos.length<n) p.pagamentos.push({status:'pendente',dataPagamento:null,valorPago:null}); fix=true; }
    }
    if(fix) this.savePar(a);
    return a;
  },
