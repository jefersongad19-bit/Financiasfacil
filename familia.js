/* ═══════════════════════════════════════════════════════════════
   FAMÍLIA AO VIVO — Firestore (módulo isolado, não toca no resto)
   ───────────────────────────────────────────────────────────────
   ▶▶▶  COLE AQUI a config do seu projeto Firebase (financasfacil-e0906)
        Console Firebase → ⚙ Configurações do projeto → Seus apps
        → app Web → objeto firebaseConfig. Troque os "COLE_AQUI".
   ▶▶▶  Ative tambÉm: Authentication → Sign-in method → Anônimo (Ativar)
   ▶▶▶  Regras do Firestore: ver o arquivo de instruções entregue junto.
   ═══════════════════════════════════════════════════════════════ */
const FIREBASE_CONFIG = {
  apiKey: "COLE_AQUI",
  authDomain: "financasfacil-e0906.firebaseapp.com",
  projectId: "financasfacil-e0906",
  storageBucket: "financasfacil-e0906.appspot.com",
...
(function(){
  ...
  function _cfgOk(){ return !!(FIREBASE_CONFIG && FIREBASE_CONFIG.apiKey && FIREBASE_CONFIG.apiKey.indexOf('COLE') === -1); }
  ...
  async function initFb(){
    ...
    _fb = { app, auth, db, uid: cred.user.uid, api: fsMod };
    return _fb;
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    const st = getState();
    if(st && st.code && _cfgOk()){ subscribe(st.code); pushMeu().catch(function(){}); }
  });
})();
