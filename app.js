...
function startApp(){
  // Aguarda Firebase Auth resolver antes de mostrar app ou tela de login
  FFAuth.onReady(user => {
...
function _ffShowApp(){
  document.getElementById('ffLoginScreen').classList.add('hidden');
  document.getElementById('app').classList.remove('hidden');
  // Callback de sync em tempo real (re-renderiza ao receber dados do Firestore)
  window._ffOnSync = () => {
...
function _ffShowLogin(){
  document.getElementById('app').classList.add('hidden');
  document.getElementById('ffLoginScreen').classList.remove('hidden');
}
...
async function ffLogout(){
  ...
  await FFAuth.logout();
  // Limpa cache local ao sair
  ['tx','par','metas','sonhos','cfg','fam_members','prev_mes','prev_trimestre','prev_ano']
    .forEach(k => localStorage.removeItem('ff2_'+k));
  _ffShowLogin();
}
